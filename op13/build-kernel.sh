...
# ===== 非交互固定参数 (ReSukiSU + susfs + KPM, Arch/LLVM=1 适配) =====
echo "===== 欧加真SM8750通用6.6.66 A15 OKI内核 本地编译 (Arch/LLVM=1 适配) ====="
MANIFEST=${MANIFEST:-oppo+oplus+realme}
CUSTOM_SUFFIX="android15-8-g29d86c5fc9dd-abogki428889875-4k"
APPLY_SUSFS=y
USE_PATCH_LINUX=y       # KPM: KpatchNext(KPN)
...
echo "启用susfs: $APPLY_SUSFS   启用KPM(KPN): $USE_PATCH_LINUX"
echo "lz4&zstd: $APPLY_LZ4   网络增强: $APPLY_BETTERNET   ADIOS: $APPLY_ADIOS   基带保护: $APPLY_BBG"
echo "===================="
echo
...
# ===== 构建依赖 (Arch / pacman) =====
echo ">>> 安装构建依赖 (pacman)..."
SU pacman -Sy --needed --noconfirm aria2 flex bison openssl bc cpio zip unzip perl python3 git wget curl rsync make || true

# ===== cctv18 预编译 Clang 18 (r510928) + build-tools (含 pahole, 供 BTF) =====
TC_DIR="$SCRIPT_DIR/toolchain"
mkdir -p "$TC_DIR"
if [ ! -x "$TC_DIR/clang18/bin/clang" ]; then
  echo ">>> 下载预编译 Clang 18 (r510928)..."
  aria2c -s16 -x16 -k1M -d "$TC_DIR" -o clang.zip "https://github.com/cctv18/oneplus_sm8650_toolchain/releases/download/LLVM-Clang18-r510928/clang-r510928.zip"
  rm -rf "$TC_DIR/clang18"; mkdir -p "$TC_DIR/clang18"
  unzip -q "$TC_DIR/clang.zip" -d "$TC_DIR/clang18"; rm -f "$TC_DIR/clang.zip"
fi
if [ ! -x "$TC_DIR/build-tools/bin/pahole" ]; then
  echo ">>> 下载 build-tools (含 pahole)..."
  aria2c -s16 -x16 -k1M -d "$TC_DIR" -o build-tools.zip "https://github.com/cctv18/oneplus_sm8650_toolchain/releases/download/LLVM-Clang18-r510928/build-tools.zip"
  ( cd "$TC_DIR" && unzip -q build-tools.zip && rm -f build-tools.zip )
fi
export PATH="$TC_DIR/clang18/bin:$TC_DIR/build-tools/bin:$PATH"
echo ">>> clang : $(clang --version | head -1)"
echo ">>> pahole: $(pahole --version 2>/dev/null || echo 'MISSING (BTF会被静默关->bootloop)')"

# ===================================================================
# 以下为原 builder_6.6.66.sh 第 89 行起内容 (LLVM=-18->LLVM=1, sudo sed->sed)
# ===================================================================
# ===== 初始化仓库 =====
echo ">>> 初始化仓库..."
...
# ===== 修复: 去掉 bpf 工具 -Werror (clang 编 libbpf.c 撞 discards-qualifiers) =====
echo ">>> 去除 tools/lib/bpf 的 -Werror..."
sed -i "s/-Werror -Wall/-Wall/" ./common/tools/lib/bpf/Makefile || true

# ===== 编译内核 =====
echo ">>> 开始编译内核..."
cd common
# ===== inject self-signed manager trusted cert (ReSukiSU Custom slot) =====
# kernel/Kbuild(214-221) turns KSU_EXPECTED_SIZE/HASH into -DEXPECTED_SIZE/-DEXPECTED_HASH;
# apk_sign.c appends {EXPECTED_SIZE,EXPECTED_HASH} under CONFIG_KSU_MULTI_MANAGER_SUPPORT=y (default y in this tree).
# Values = DER cert byte size + sha256(lowercase hex) of the keystore cert that signed the self-signed manager APK (dR).
export KSU_EXPECTED_SIZE=785
export KSU_EXPECTED_HASH=3374a2760b60fb189a0ef24f8ca3a9b2af60819654ce6e633a42b03dd73cbfdb
echo ">>> self-signed manager trusted cert injected: size=$KSU_EXPECTED_SIZE hash=$KSU_EXPECTED_HASH"

make -j$(nproc --all) LLVM=1 ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- CROSS_COMPILE_ARM32=arm-linux-gnuabeihf- CC=clang LD=ld.lld HOSTCC=clang HOSTLD=ld.lld O=out KCFLAGS+=-O2 KCFLAGS+=-Wno-error KSU_EXPECTED_SIZE=$KSU_EXPECTED_SIZE KSU_EXPECTED_HASH=$KSU_EXPECTED_HASH gki_defconfig Image
echo ">>> 内核编译成功！"
